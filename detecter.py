...
def foo(path):
    ...
    # Model Files
    modelConfiguration = r"yolo\yolov4.cfg"
    modelWeigheights = r"yolo\yolov4.weights"
    ...
    def realTime():
        while True:
            ...
            cv2.line(img, (0, middle_line_position),
                     (iw, middle_line_position), (0, 0, 255), 3)
            # cv2.rectangle(img, (5, 3), (225, 125), (255,255,255), cv2.FILLED)
            x, y, w, h = 5, 3, 290, 175
            sub_img = img[y:y + h, x:x + w]
            white_rect = np.ones(sub_img.shape, dtype=np.uint8) * 12
            ...
            cv2.putText(img, "     Down", (160, 20), cv2.FONT_HERSHEY_SIMPLEX,
                        font_size, font_color, font_thickness)
            cv2.putText(img, "     ", (160, 50), cv2.FONT_HERSHEY_SIMPLEX,
                        font_size, font_color, font_thickness)
            cv2.putText(img, "Car:        "+str(up_list[0])+"     " + str(
                down_list[0]), (20, 50), cv2.  FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
            cv2.putText(img, "Motorbike:  "+str(up_list[1])+"     " + str(
                down_list[1]), (20, 80), cv2.  FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
            cv2.putText(img, "Bus:        "+str(up_list[2])+"     " + str(
                down_list[2]), (20, 110), cv2.  FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
            cv2.putText(img, "Truck:      "+str(up_list[3])+"     " + str(
                down_list[3]), (20, 140), cv2. FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
            cv2.putText(img, "Total:      "+str(up_list[0]+up_list[1]+up_list[2]+up_list[3])+"     " + str(
                down_list[0]+down_list[1]+down_list[2]+down_list[3]), (20, 170), cv2. FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
